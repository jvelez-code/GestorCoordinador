...
export class ExcelTmoEntranteSalienteService {
  ...
  async tmoEntranteSaliente(dataExcel: ReportesGeneral[], parametros: any): Promise<void> {
    //Configuraciones generales del excel
    this._worbook = new Workbook();
    this._worbook.creator = "JaimeTorres";

    await this.crearEntranteSaliente(dataExcel, parametros);

    this._worbook.xlsx.writeBuffer().then((data) => {
      const blob = new Blob([data]);
      fs.saveAs(blob, "Reporte_tmo_entrante_saliente.xlsx");
    });
  }

  //creamos un nuevo reporte con los datos de la tabla y los parametros
  private crearEntranteSaliente(dataExcelTable: ReportesGeneral[], parametros: any): void {
    ...
    const sheet = this._worbook.addWorksheet(`REPORTE TMO ENTRANTE SALIENTE ${empresa}`);
    ...
    ["A", "B", "C", "D", "E", "F", "G", "H", "I", "J", "K"].forEach((columkey) => {
    ...
    sheet.addImage(logoidjaime, "F10:C16");
    sheet.addImage(logojtcnegro, "G2:D5");
    ...
    //sheet.mergeCells(`A10000:K${cf + 1}`); //combina las celdas para dejar el espacio blanco abajo
    sheet.mergeCells("L10000:BD1"); //combina las celdas para dejar el espacio blanco a la derecha

    //Nombre visible del reporte
    sheet.getCell("A1").value = `REPORTE TMO ENTRANTE SALIENTE ${empresa}`;
    sheet.mergeCells("C2:K5"); //combinar celdas para logo jtc negro
    ...
    for (let index = 0; index < rowsToInsert.length; index++) {
      ...
      row.values = [
        itemData.documentofinal,
        itemData.agentefinal,
        itemData.entrante,
        itemData.cantidade,
        itemData.duracione,
        itemData.saliente,
