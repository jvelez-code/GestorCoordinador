...
export class ExcelLlamadascalificadasService {

  constructor() { }
  private _worbook!: Workbook;


  //#################################### REPORTE Llamadas calificadas #######################################################

  //Recibimos la data que es el reporte y los parametros desde donde se origina el reporte
  async llamadascalificadas(dataExcel: ReportesGeneral[], parametros: any): Promise<void> {
    //Configuraciones generales del excel
    this._worbook = new Workbook();
    this._worbook.creator = "JaimeTorres";

    await this.crearLlamadasCalificadas(dataExcel, parametros);

    this._worbook.xlsx.writeBuffer().then((data) => {
      const blob = new Blob([data]);
      fs.saveAs(blob, "Reporte_llamadas_calificadas.xlsx");
    });
  }

  //creamos un nuevo reporte con los datos de la tabla y los parametros
  private crearLlamadasCalificadas(dataExcelTable: ReportesGeneral[], parametros: any): void {
    ...
    const sheet = this._worbook.addWorksheet(`REPORTE LLAMADAS CALIFICADAS ${empresa}`);
    ...
    sheet.getCell("A1").value = `REPORTE LLAMADAS CALIFICADAS ${empresa}`;
    ...
    for (let index = 0; index < rowsToInsert.length; index++) {
      ...
      row.values = [
        itemData.tipo_doc,
        itemData.numero_documento,
        itemData.calificacion,
        itemData.idasterisk,
        itemData.documentoagente,
        itemData.agente,
      ];
    }
