...
export class ExcelSecretariaVirtualService {
  ...
  async secretariaVirtual(dataExcel: ReportesGeneral[], parametros: any): Promise<void> {
    //Configuraciones generales del excel
    this._worbook = new Workbook();
    this._worbook.creator = "JaimeTorres";

    await this.crearSecretariaVirtual(dataExcel, parametros);

    this._worbook.xlsx.writeBuffer().then((data) => {
      const blob = new Blob([data]);
      fs.saveAs(blob, "Reporte_secretaria_virtual.xlsx");
    });
  }

  //creamos un nuevo reporte con los datos de la tabla y los parametros
  private crearSecretariaVirtual(dataExcelTable: ReportesGeneral[], parametros: any): void {
    ...
    const sheet = this._worbook.addWorksheet(`REPORTE SECRETARIA VIRTUAL ${empresa}`);
    ...
    sheet.addImage(logoidjaime, "D8:B17");
    ...
    sheet.getCell("A1").value = `REPORTE SECRETARIA VIRTUAL ${empresa}`;
