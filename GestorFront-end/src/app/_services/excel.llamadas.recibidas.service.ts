...
export class ExcelLlamadasRecibidasService {
  ...
  async llamadasrecibidas(dataExcel: ReportesGeneral[], parametros: any): Promise<void> {
    //Configuraciones generales del excel
    this._worbook = new Workbook();
    this._worbook.creator = "JaimeTorres";

    await this.crearLlamadasRecibidas(dataExcel, parametros);
  ...
  private crearLlamadasRecibidas(dataExcelTable: ReportesGeneral[], parametros: any): void {
    ...
    const sheet = this._worbook.addWorksheet(`REPORTE LLAMADAS RECIBIDAS ${empresa}`);
    ...
    sheet.addImage(logoidjaime, "E8:B26");
    ...
    for (let index = 0; index < rowsToInsert.length; index++) {
      ...
      row.values = [
        itemData.fecha_llamada,
        itemData.atendidas,
        itemData.calif_positiva,
        itemData.calif_negativo,
        itemData.total_llamadas,
        itemData.telefono,
      ];
    }
