...
export class ExcelTmoDetalladoService {

  constructor() { }
  private _worbook!: Workbook;
  async tmoDetallado(dataExcel: ReportesGeneral[], parametros: any): Promise<void> {
    //Configuraciones generales del excel
    this._worbook = new Workbook();
    this._worbook.creator = "JaimeTorres";

    await this.crearTmoDetallado(dataExcel, parametros);

    this._worbook.xlsx.writeBuffer().then((data) => {
      const blob = new Blob([data]);
      fs.saveAs(blob, "reporteTmoDetallado.xlsx");
    });
  }

  //creamos un nuevo reporte con los datos de la tabla y los parametros
  private crearTmoDetallado(dataExcelTable: ReportesGeneral[], parametros: any): void {
    ...
    const sheet = this._worbook.addWorksheet(`REPORTE TMO DETALLADO ${empresa}`);
    ...
    sheet.addImage(logoidjaime, "E10:A27");
    ...
    sheet.mergeCells(`A10000:E${cf + 1}`); //combina las celdas para dejar el espacio blanco abajo
    sheet.mergeCells("F10000:AH1"); //combina las celdas para dejar el espacio blanco a la derecha

    //Nombre visible del reporte
    sheet.getCell("A1").value = `REPORTE TMO DETALLADO ${empresa}`;
