...
class ReporGestor {
    ...
    static postDetalleGestiones = async (req, res) => {
        try {
            ...
            const response = await pool.query(`SELECT
                    ...
                    WHERE  g.fecha_gestion BETWEEN ($1) AND ($2) AND c.id_empresa=($3) 
                    ORDER BY g.fecha_gestion`, [fechaini, fechafin, empresa]);
    ...
    static postDetalleGestionesUsuario = async (req, res) => {
        try {
            const fechaActual = moment().tz('America/Bogota').format('YYYY-MM-DD HH:mm:ss');
            ...
            const response = await pool.query(`SELECT
                    ...
                    LEFT JOIN usuario ag ON dg.id_agente=ag.id_usuario
                    LEFT JOIN empresa emp ON ag.empresa=emp.id_empresa
                    WHERE  g.fecha_gestion BETWEEN ($1) AND ($2) AND c.id_empresa=($3) AND g.id_agente=($4)
                    ORDER BY g.fecha_gestion`, [fechaini, fechafin, idEmpresa, idUsuario]);
    ...
    static postVisitaComercial = async (req, res) => {
        try {
            ...
            CL.direccion,
            TO_CHAR((SC.fecha_ingreso_visita AT TIME ZONE 'UTC' AT TIME ZONE 'America/Bogota' + INTERVAL '5 hours'), 'YYYY-MM-DD') as fecha_ingreso_visita,
            TO_CHAR((SC.fecha_salida_visita AT TIME ZONE 'UTC' AT TIME ZONE 'America/Bogota' + INTERVAL '5 hours'), 'YYYY-MM-DD') as fecha_salida_visita,
            TO_CHAR((SC.fecha_gestion_seguimiento AT TIME ZONE 'UTC' AT TIME ZONE 'America/Bogota' + INTERVAL '5 hours'), 'YYYY-MM-DD HH24:MI:SS') as fecha_gestion_seguimiento,
            USU.usuario,
            SC.operador_actual,
            SC.encuesta_n1,
            SC.encuesta_n2,
            ...
            CC.compromiso,
            CC.responsable,
            TO_CHAR((CC.fecha_compromiso AT TIME ZONE 'UTC' AT TIME ZONE 'America/Bogota' + INTERVAL '5 hours'), 'YYYY-MM-DD') as fecha_compromiso,
            SC.referidos_empresa,
            SC.referidos_contacto,
            SC.referidos_email,
            SC.referidos_telefono,
            TO_CHAR((SC.fecha_liquidacion_pago AT TIME ZONE 'UTC' AT TIME ZONE 'America/Bogota' + INTERVAL '5 hours'), 'YYYY-MM-DD') as fecha_liquidacion_pago
       FROM
            seguimiento_comercial SC 
            INNER JOIN gestion G ON SC.id_gestion = G.id_gestion
            LEFT JOIN compromiso_comercial CC ON SC.id_seguimiento = CC.id_seguimiento_comercial
            INNER JOIN estado_gestion EG ON G.id_estado_gestion = EG.id_estado_gestion
            INNER JOIN detalle_gestion DG ON G.id_gestion = DG.id_gestion 
            INNER JOIN cliente CL ON G.id_cliente = CL.id_cliente
            INNER JOIN contacto CO ON G.id_gestion = CO.id_gestion 
            INNER JOIN usuario USU ON G.id_agente = USU.id_usuario
            INNER JOIN divipola MUN ON CL.id_zona = MUN.id_zona
            INNER JOIN divipola DEP ON MUN.id_zona_padre = DEP.id_zona
       WHERE SC.fecha_gestion_seguimiento BETWEEN ($1) and ($2) 
       ORDER BY fecha_gestion_seguimiento DESC `, [fechaini, fechafin]);
    ...
    static postVisitaComercialUsuario = async (req, res) => {
        try {
       ...
       FROM
            ...
            INNER JOIN divipola MUN ON CL.id_zona = MUN.id_zona
            INNER JOIN divipola DEP ON MUN.id_zona_padre = DEP.id_zona
       WHERE SC.fecha_gestion_seguimiento BETWEEN ($1) and ($2) AND usu.id_usuario =($3)
       ORDER BY fecha_gestion_seguimiento DESC`, [fechaini, fechafin, idUsuario]);
    ...
    static postFidelizacionComercial = async (req, res) => {
        try {
            ...
            FID.fecha_pago as fecha_pago,
            FID.numero_planilla, FID.observacion, FID.migracion,
            to_char(FID.fecha_gestion, 'YYYY-MM-DD HH24:MI:SS') as fecha_gestion
            FROM fidelizacion_comercial FID, detalle_gestion_comercial dgc ,gestion g , cliente CLI, usuario USU
            WHERE FID.id_detalle_gestion_comercial= dgc.id_detalle_gestion_comercial
            AND dgc.id_gestion= g.id_gestion
            AND g.id_cliente = CLI.id_cliente 
            AND g.id_agente = USU.id_usuario 
            AND FID.fecha_gestion BETWEEN ($1) and ($2)
    ...
    static postFidelizacionUsuario = async (req, res) => {
        try {
            ...
            AND g.id_cliente = CLI.id_cliente 
            AND g.id_agente = USU.id_usuario 
            AND FID.fecha_gestion BETWEEN ($1) and ($2) AND usu.id_usuario =($3)
            ORDER BY FID.fecha_gestion`, [fechaini, fechafin, idUsuario]);
    ...
    static postReporteCasos = async (req, res) => {
        try {
            let fechaini = req.body.fechaini;
            let fechafin = req.body.fechafin;
            ...
            SELECT 
                cs.id_caso, dt.id_detalle, cs.fecha_caso AS fecha_creacion, cf.nombre_fuente , c.usuario_act AS usuario, c.telefono_celular, cs.nro_realmarcado, c.correo,dp.nombre_departamento,dv.nombre as ubicacion, c.tipo_documento, c.nro_documento, ctgr.nombre_categoria, csubc.nombre_subcategoria,
                ctpl.nombre_tipologia , cps.nombre_proceso ,nv.nombre_nivel,dt.fecha_detalle AS ultima_modificacion, dt.observacion, cs.fecha_cierre, cs.fecha_vencimiento,
                ctpl.nombre_tipologia, c.id_cliente, c.id_zona, c.razon_social, c.fecha_hora_sis, c.id, (now()::date - cs.fecha_caso::date) as dias_de_gestion, ctgr.dias_vencimiento, est.nombre_estado as estado
            FROM  
                crm_casos cs
            JOIN 
                crm_detalle dt ON dt.id_caso = cs.id_caso
            JOIN 
                cliente c ON c.id_cliente = cs.id_cliente
            LEFT JOIN 
                crm_tipologia ctpl ON cs.id_tipologia = ctpl.id_tipologia
            LEFT JOIN 
                crm_subcategoria csubc ON ctpl.id_subcategoria = csubc.id_subcategoria
            LEFT JOIN 
                crm_categoria ctgr ON csubc.id_categoria = ctgr.id_categoria
            LEFT JOIN
                crm_departamento dp ON dp.id_departamento = cs.id_departamento
            LEFT JOIN
                crm_proceso cps ON cps.id_proceso = cs.id_proceso
            LEFT JOIN
                crm_nivel nv ON nv.id_nivel  = cs.id_nivel
            LEFT JOIN
                crm_estado est ON est.id_estado = cs.id_estado 
            LEFT JOIN
                crm_fuente cf  ON cf.id_fuente  = cs.id_fuente
            LEFT JOIN
                divipola dv  ON dv.id_zona  = c.id_zona
            WHERE 
                cs.fecha_caso BETWEEN $1 AND $2
            ORDER BY 
                cs.fecha_caso`, [fechaini, fechafin]);
